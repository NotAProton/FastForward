...
const ODP = (t, p, o) => {
  ...
  URL = window.URL,
  docSetAttribute = document.documentElement.setAttribute.bind(
  ...
  isGoodLink = link => {
    if (
      ...
      (link.split('#')[0] == location.href.split('#')[0] &&
        !isGoodLink_allowSelf) ||
      link.substr(0, 6) == 'about:' ||
  ...
  unsafelyNavigate = target => {
    ...
    let url =
      'https://universal-bypass.org/bypassed?target=' +
  ...
  unsafelyAssignWithReferer = (target, referer) => {
    ...
    location.href =
      'https://universal-bypass.org/navigate?target=' +
      encodeURIComponent(target) +
      '&referer=' +
      encodeURIComponent(referer)
  },
...
;(domainBypass = (domain, f) => {
  ...
  if (typeof domain == 'string') {
    if (
      ...
      location.hostname.substr(
        location.hostname.length - (domain.length + 1)
      ) ==
        '.' + domain
    ) {
      bypassed = true
  ...
  (crowdBypass = (f, a) => {
    ...
    if (crowdEnabled) {
      if (ignoreCrowdBypass) {
        ...
        let iT = setInterval(() => {
          if (
            document.documentElement.hasAttribute('{{channel.crowd_queried}}')
          ) {
            clearInterval(iT)
            document.documentElement.removeAttribute(
              '{{channel.crowd_queried}}}'
            )
  ...
  (insertInfoBox = text =>
    ensureDomLoaded(() => {
      ...
      div.innerHTML =
        '<img src="{{icon/48.png}}" style="width:24px;height:24px;margin-right:8px"><span style="display:inline"></span>'
...
//Safelink
let forced_safelink = { counter: 0, adblock: false, click2x: false },
  actual_safelink = forced_safelink
ODP(window, 'safelink', {
  set: _ => {
    for (let k in _) {
...
domainBypass(/ur\.ly|urly\.mobi/, () => {
  if (
    ...
    location.pathname.substr(0, 6) != '/goii/'
  ) {
    safelyNavigate(
      '/goii/' +
...
hrefBypass(/sourceforge\.net\/projects\/.+\/files\/.+\/download/, () => {
  ...
  ODP(window, 'log', {
    value: m => {
      console.log(m)
      if (m == 'triggering downloader:start') d = true
    },
    writable: false
  })
  ensureDomLoaded(() => {
...
domainBypass('tei.ai', () => {
  ensureDomLoaded(() => {
    var link = atob(
      'aH' +
        document.querySelector("#link-view [name='token']").value.split('aH')[1]
    )
    safelyNavigate(link)
  })
})
...
domainBypass(/mylinks\.xyz|mylink\.zone|mylink1\.biz/, () => {
  //clictune
  window.setTimeout = f => setTimeout(f, 1)
  awaitElement('#compteur a[href]', a =>
    safelyNavigate(new URL(a.href).searchParams.get('url'))
  )
})
...
domainBypass('hokiciki.org', () =>
  ifElement("a[href^='/get-link/']", safelyAssign)
)
domainBypass(
  /wadooo\.com|gotravelgo\.space|pantauterus\.me|liputannubi\.net/,
  () => {
    crowdPath(location.hash.substr(1))
    crowdBypass()
  }
)
domainBypass('lnk.news', () =>
  ifElement(
    '#display_go_form',
    f => {
      window.open = () => {}
...
hrefBypass(
  /(prox77|agdd5br)\.com\/analyze\/(.+)/,
  m => (location.pathname = '/result/' + m[2])
)
  ...
  ODP(window, 'url_page', {
    get: () => {
      safelyNavigate(
        Aes.Ctr.decrypt(
          location.href
            .substr(location.href.indexOf('?hash=') + 6)
            .split('&')[0],
          '-encrypt download@ip3enk',
          256
        )
      )
    }
...
domainBypass(
  /^((www\.)?((safe\.anirocksite|my-code4you\.blogspot|jemerik)\.com|busyfinance\.site|behealth-id\.xyz))$/,
  () => {
      ...
      ensureDomLoaded(() => {
        let w = setInterval(() => {
          if (typeof CryptoJS != 'undefined') {
            clearInterval(w)
            let u,
              m = p.get(p.has('safe') ? 'safe' : 'kareeI')
            try {
              u = CryptoJS.AES.decrypt(m, 'CryptoHEXKareela2FyZWVsa3Vu', {
                format: {
                  parse: t => {
              ...
              u = JSON.parse(
                CryptoJS.AES.decrypt(m, 'CryptoJSAESpass').toString(
                  CryptoJS.enc.Utf8
                )
              ).url
            }
...
domainBypass('cheatsquad.gg', () => {
  ...
  ensureDomLoaded(() =>
    document
      ...
      .forEach(d => (d.className = 'check_loader'))
  )
})
...
domainBypass(/pahe\.(in|me|ph)/, () => {
        ...
        eval(
          '(' +
            f
              ...
              .split('[n*15];')
              .join(
                "[n*15]+'#bypassClipboard='+event.target.getAttribute('data-bypass-clipboard')"
              ) +
            ')()'
        )
      )
  ...
  ensureDomLoaded(() =>
    document.querySelectorAll('acee').forEach(a => {
      ...
      if (qe !== null) {
        s += (qe.tagName == 'BR' ? qe.previousSibling : qe).textContent
          .replace(/[^a-zA-Z0-9]/g, '')
          .toLowerCase()
      }
...
domainBypass('boost.ink', () =>
  ifElement('script[version]', s =>
    safelyNavigate(atob(s.getAttribute('version')))
  )
)
domainBypass('linksunlocked.com', () => {
  ...
  if (searchParams.has('token')) {
    safelyNavigate(
      'https://uploadhaven.com/download/' + searchParams.get('token')
    )
  }
})
domainBypass('samehadaku.vip', () =>
  ifElement('div.close-button', a => a.click())
)
domainBypass('playhindi.com', () =>
  transparentProperty('downloadButton', safelyNavigate)
)
...
hrefBypass(
  /(bluemediafiles\.com|pcgamestorrents.org)\/url-generator\.php\?url=/,
  () => {
    window.setInterval = f => setInterval(f, 1)
...
domainBypass('downloadr.in', () =>
  safelyNavigate(new URL(location.href).search.slice(1))
)
domainBypass(
  /^((www\.)?((njiir|healthykk|linkasm|dxdrive|getwallpapers|sammobile|ydfile|mobilemodsapk|dlandroid|download\.modsofapk)\.com|(punchsubs|zedge|fex)\.net|k2s\.cc|muhammadyoga\.me|u\.to|skiplink\.io|(uploadfree|freeupload)\.info|fstore\.biz))$/,
  () => (window.setInterval = f => setInterval(f, 1))
)
hrefBypass(
  /thesimsresource\.com\/downloads\/details\/id\//,
  () => (window.setTimeout = f => setTimeout(f, 1))
)
hrefBypass(
  /firefaucet\.win\/l\/|sfirmware\.com\/downloads-file\/|(apkily\.com\/getapp$)|androidtop\.net\/\?do=downloads\&id=/,
  () => (window.setInterval = f => setInterval(f, 1))
)
hrefBypass(
  /emulator\.games\/download\.php|curseforge\.com\/.*\/download\/[0-9]*/,
  () => (window.setInterval = f => setInterval(f, 100))
)
domainBypass(
  /^((www\.)?((racaty|longfiles|filepuma|portableapps)\.com|indishare\.org|datei\.to|keisekai\.fun|solvetube\.site))$/,
  () => (window.setTimeout = f => setTimeout(f, 1))
)
domainBypass(
  /lkc21\.net|layarkacaxxi\.org/,
...
domainBypass(/linkvertise\.(com|net)|link-to\.net/, () => {
  ...
  // normal redirect
  fetch(
    `https://publisher.linkvertise.com/api/v1/redirect/link/static${location.pathname}`
  )
    .then(r => r.json())
    .then(json => {
...
ensureDomLoaded(() => {
  ...
  domainBypass(
    /^((www\.)?((file(factory|-upload)|asdfiles|mega4up)\.com|up-load\.io|cosmobox\.org|rockfile\.co|devdrive\.cloud))$/,
    () => insertInfoBox('{{msg.infoFileHoster}}')
  )
  domainBypass(/linkvertise\.(com|net)|link-to\.net/, () =>
    insertInfoBox(
      UNIVERSAL_BYPASS_INTERNAL_VERSION >= 9
        ? '{{msg.infoLinkvertise}}'
        : "We're not allowed to bypass this website but we have negotiated the removal of their most annoying steps."
    )
  )
  domainBypass('srt.am', () => {
  ...
  domainBypass('complete2unlock.com', () => {
    let bT = setInterval(() => {
      let b = document.getElementById('link-success-button'),
        es = document.querySelectorAll('.unlockpanel')
      if (b && es.length > 0) {
        clearInterval(bT)
        window.open = () => {}
  ...
  domainBypass('won.pe', () =>
    ifElement('#progress', p => {
      p.setAttribute('aria-valuenow', '100')
      awaitElement("#skip_button[href]:not([href=''])", b =>
        safelyNavigate(window.longURL)
      )
    })
  )
  domainBypass('gotoo.loncat.in', () =>
    ifElement("a[href^='http://gotoo.loncat.in/go.php?open=']", safelyNavigate)
  )
  ...
  domainBypass('rom.io', () =>
    crowdBypass(() =>
      awaitElement('a.final-button[href]', a => {
        if (isGoodLink(a.href)) {
  ...
  hrefBypass(/(4snip\.pw|flare\.icu)\/(out|decode)\//, () => {
    ...
    f.setAttribute(
      'action',
      f.getAttribute('action').replace('../out2/', '../outlink/')
    )
    countIt(() => f.submit())
  })
  domainBypass('elsfile.org', () => {
    ...
    f.method = 'POST'
    f.innerHTML =
      '<input type="hidden" name="op" value="download1"><input type="hidden" name="usr_login" value="C"><input type="hidden" name="id" value="' +
      location.pathname.substr(1) +
      '"><input type="hidden" name="fname"><input type="hidden" name="referer" value="q"><input type="hidden" name="method_free" value="Free Download">'
    f.querySelector("[name='fname']").value = document.querySelectorAll(
      'div#container > div > div > table > tbody > tr > td'
    )[2].textContent
    f = document.documentElement.appendChild(f)
    countIt(() => f.submit())
  })
  domainBypass(/goou\.in|manualsbooks\.com/, () =>
    ifElement('div#download_link > a#download[href]', safelyNavigate)
  )
  domainBypass('ryn.cc', () => {
  ...
  domainBypass('drivehub.link', () =>
    ifElement('a#proceed[href]', safelyNavigate)
  )
  domainBypass('oxy.st', () => {
    awaitElement('a.btn.btn-primary.btn-lg', t => {
      safelyAssign(t.href)
    }),
      ifElement('button#download[disabled]', t => {
        awaitElement('button#download:not([disabled])', t => {
          t.click()
        })
      })
  })
  domainBypass('oxy.cloud', () => {
    ifElement('button#download[disabled]', d => {
      awaitElement('button#download:not([disabled])', d => {
        d.click()
      })
    })
  })
  domainBypass('daunshorte.teknologilink.com', () =>
    safelyAssign(
      document.querySelector(
        "a[href^='https://teknosafe.teknologilink.com/linkteknolink/safelinkscript.php?']"
      ).href
    )
  )
  ...
  domainBypass(/sub2unlock\.(com|net)/, () =>
    safelyNavigate(document.getElementById('theGetLink').textContent)
  )
  domainBypass('boostme.gg', () =>
    safelyNavigate(document.querySelector('a[href]#go').href)
  )
  domainBypass(
    /(driverays|bioskopgo|01nonton|thetecnostar|curimovie|akltu)\.com|cinema21\.tv/,
    () => ifElement('a#link[href]', safelyAssign)
  )
  domainBypass('wikitrik.com', () =>
    document
      .querySelector(
        "#download > form[action='/getlink.php'] > input[type='submit'].button"
      )
      .click()
  )
  domainBypass('dawnstation.com', () =>
    safelyNavigate(
      document.querySelector('#tidakakanselamanya.hiddenPlace > a').href
    )
  )
  domainBypass('hokiwikiped.net', () =>
    ifElement('a#DrRO[href]', safelyNavigate)
  )
  hrefBypass(/spaste\.com\/(s|site)\//, () => {
    ...
    doTheThing(() =>
      doTheThing(() =>
        doTheThing(() =>
          document.querySelector('#template-contactform-submit').click()
        )
      )
    )
  })
  domainBypass(
    /^((www\.)?(((get-click2|informations-library|media-blue|akashirohige|aibouanimelink|wwwfotografgotlin|casperqu|safelinksencrypter)\.blogspot|business\.ominfoupdate|majidzhacker|citgratis|tekloggers|pro-bangla|ph\.(apps2app|samapkstore)|blog\.(hulblog|omgmusik|omglyrics))\.com|(pastikan|belajar-bersama2)\.me|ph\.tpaste\.net|(blog\.infolanjutan|jkoding)\.xyz|((safe\.onbatch|anonimfiles)\.my|google-playss\.sdetectives)\.id|jackofnine\.site|getlink\.animesanka\.club))$/,
    () => {
      let u = aesCrypto.decrypt(
        convertstr(location.href.substr(location.href.indexOf('?o=') + 3)),
        convertstr('root')
      )
      if (isGoodLink(u)) {
        location.hash = ''
        ...
        u = aesCrypto.decrypt(
          convertstr(uri.substr(uri.indexOf('?o=') + 3)),
          convertstr('root')
        )
        safelyNavigate(u)
        ...
        u = aesCrypto.decrypt(
          convertstr(location.href.substr(location.href.indexOf('#go') + 3)),
          convertstr('root')
        )
        location.hash = ''
  ...
  domainBypass('hello.tribuntekno.com', () =>
    ifElement('#splash p[style] > u > b > a[href]', safelyNavigate)
  )
  ...
  domainBypass('shortlink.in', () => {
    ifElement(
      '#continueButton.interContinueButton > a[href]',
      a => {
        safelyNavigate(a.href)
      },
      () =>
        ifElement("frame[src^='interstitualAdTop.php?url=']", f =>
          safelyAssign(f.src)
        )
    )
  })
  domainBypass('safe.doramaku.me', () => {
    document
      .querySelector('form')
      .setAttribute('action', '//' + location.hostname)
    document.querySelector('input').setAttribute('name', 'link')
  })
  domainBypass('confile.net', () => {
    ifElement(
      "#verif > form [type='submit']",
      b => b.click(),
      () => {
        if (
          typeof downloadButton == 'object' &&
  ...
  domainBypass(/linkshrtn\.com|(xtiny|shrten)\.link/, () => {
    ...
    window.setTimeout = f => setTimeout(f, 1)
    window.setInterval = f => setInterval(f, 1)
    awaitElement('.complete_btn:not([disabled])', b =>
      b.onclick({ isTrusted: 1 })
    )
    document
      .querySelectorAll('.all_steps > .step_block:not(.step_done)')
      .forEach(e => {
  ...
  domainBypass(/linkpoi\.(in|cc|me)/, () =>
    ifElement('a.btn.btn-primary[href]', safelyNavigate)
  )
  domainBypass(/spacetica\.com|linegee\.net/, () =>
    setTimeout(() => {
      let links = []
      document
        .querySelectorAll('a.btn[href],a.btn-primary[href],a.btn-xs[href]')
        .forEach(a => {
          links.push(a.href)
      ...
      for (let i = 0; i < links.length; i++) {
        if (
          links[i].indexOf('google.com/search') == -1 &&
          links[i].indexOf('/404') == -1 &&
          !/^https?:\/\/.+\/[0-9a-f]{40,}$/i.exec(links[i])
        ) {
          true_i = true_i == -1 ? i : -2
        }
      }
  ...
  domainBypass(/uiz\.(io|app)|moon7\.xyz/, () =>
    crowdBypass(() => {
  ...
  hrefBypass(/mirrored\.to\/(down|get)link\//, () =>
    ifElement('.centered.highlight a[href]', safelyNavigate)
  )
  hrefBypass(
  ...
  domainBypass(
    /tr\.link|movienear\.me|lewat\.club|tautan\.pro|(droidtamvan|gubukbisnis|onlinecorp)\.me|(liveshootv|modebaca|haipedia|sekilastekno|miuiku)\.com|shrink\.world|link\.mymastah\.xyz|(sportif|cararoot)\.id|(fcdot|fcc)\.lc/,
    () => {
  ...
  domainBypass('lompat.in', () => {
    window.open = u => {
      if (
        u.substr(0, 28) == 'http://henpoi.lompat.in/?go=' &&
        u.substr(-4) == '&s=1'
      ) {
  ...
  domainBypass('short-url.link', () =>
    safelyNavigate(
      document.querySelector('div[align=center] > strong').textContent
    )
  )
  domainBypass(/uploadrar\.(com|net)/, () => {
    ifElement('#downloadbtn', () => {
      ...
      f.method = 'POST'
      f.innerHTML =
        '<input name="op" value="download2"><input name="id" value="' +
        location.pathname.substr(1) +
        '">'
  ...
  domainBypass('elil.cc', () => {
    crowdBypass()
    awaitElement(
      ".navbar-custom > .container > ul.not-nav > li:not(.d-none) > a.page-scroll[href]:not([href^='javascript:'])",
      a => contributeAndNavigate(a.href)
    )
  })
  domainBypass('transmediakreatif.com', () =>
    ifElement('#download > a[href]', safelyAssign)
  )
  domainBypass(/go\.indonesia-publisher\.id|ciustekno\.me/, () => {
    if (
      typeof disqus_config == 'function' &&
      document.querySelector('form#link-view')
    ) {
      let o = { page: {} }
  ...
  hrefBypass(/mirrorace\.(com|org)\/m\/[a-zA-Z0-9]+\/[0-9]+/, () =>
    safelyAssign(
      document.querySelector(
        "a[href*='" + location.search + "']:not([hidden]):not(.uk-hidden)"
      ).href
    )
  )
  ...
  domainBypass('pucuktranslation.pw', () =>
    ifElement('#main > section > center:nth-child(2) > a:nth-child(2)', a =>
      a.click()
    )
  )
  domainBypass('gsu.st', () =>
  ...
  domainBypass('mangalist.org', () => {
    awaitElement(
      "#btt > button.btn.btn-primary.text-center[onclick^='window.location.assign(']",
      b => {
  ...
  domainBypass(/(terbit|movies)21\.[a-z]+/, () =>
    ifElement('a#downloadbutton[href]', a =>
      countIt(() => safelyAssign(a.href))
    )
  )
  domainBypass('onepieceex.net', () =>
    ifElement('noscript', n => safelyNavigate(n.textContent))
  )
  domainBypass('felanovia.com', () =>
    ifElement('form', f => countIt(() => f.submit()))
  )
  domainBypass('redir.animenine.net', () =>
    ifElement('a#lanjutkeun[href]', safelyNavigate)
  )
  ...
  hrefBypass(/(squidssh|goodssh)\.com\/li\/go\.php/, () => {
    ...
    crowdPath(p)
    crowdBypass(() =>
      ifElement("form[action='/user/links']", f => (f.action += '#' + p))
    )
  })
  domainBypass(/l\.ndoqp\.com|elnurtech\.com|jo2win\.com/, () =>
    ifElement('input#real_url', i => safelyNavigate(i.value))
  )
  ...
  hrefBypass(/stayonline\.pro\/l\/(.*)\//, m =>
    $.post(endpoint, { id: m[1], ref: '' }, r => safelyNavigate(r.data.value))
  )
  domainBypass('xlink.cc', () =>
    safelyNavigate(
      JSON.parse(atob(window.bootstrapData)).linkResponse.link.long_url
    )
  )
  ...
  domainBypass(/multiup\.(org|eu)/, () =>
    ifElement(
      "form[target][onsubmit] button[type='submit']",
      b => {
  ...
  domainBypass('mispuani.xyz', () => {
    let u = decodeURIComponent(
        location.href.substr(location.href.indexOf('?u=') + 3)
      ),
      o = JSON.parse(
        CryptoJS.AES.decrypt(u, 'MispuaniDewaGanteng').toString(
          CryptoJS.enc.Utf8
        )
      )
    safelyNavigate(o.url)
  })
  domainBypass('dl.blackmod.net', () =>
    ifElement('a.button.fa-download[href]', safelyNavigate)
  )
  domainBypass(/(bladesalvador|invistaiptv)\.com/, () =>
    ifElement(".icon > img[src^='https://api.miniature.io/?']", i => {
      let url = new URL(i.src)
      if (url.search.indexOf('url=')) {
        safelyNavigate(
          decodeURIComponent(url.search.split('url=')[1].split('&')[0])
        )
      }
    })
  )
  domainBypass(
    /bebasdownloadfilm\.com|dl\.(sharemydrive\.xyz|indexmovie\.biz)/,
    () =>
      ifElement("frame[src*='/iframe/top.php?']", f => {
        f.onload = () =>
          safelyNavigate(f.contentDocument.querySelector('p#skip a').href)
      })
  )
  domainBypass(/^((www\.)?(midvip\.xyz|midia\.vip))$/, () =>
    ifElement('button#makingdifferenttimer', b => b.onclick())
  )
  domainBypass('midiavip.com', () =>
    ifElement('button#blocklinkbtn', b => safelyNavigate(b.dataset.blurl))
  )
  domainBypass('1ink.cc', () =>
    ifElement('a#countingbtn[href]', safelyNavigate)
  )
  ...
  domainBypass('intifada1453.team', () =>
    ifElement('a.short-button[href]', safelyNavigate)
  )
  domainBypass('ahref.co', () =>
    ifElement('.download_button', a => safelyAssign(a.parentNode.href))
  )
  ...
  domainBypass(/(shon|likn)\.xyz|sloomp\.space/, () =>
    ifElement(
      "form[action*='/redirect/sgo/']",
      f => {
        if (location.pathname.substr(0, 4) == '/go/') {
          crowdPath(location.pathname.substr(4))
        }
        let s = () =>
          fetch(f.action, {
            ...
            contributeAndNavigate(r.headers.get('refresh').split('url=')[1])
          )
        if (f.querySelector("input[name='g-recaptcha-response']")) {
          awaitElement(
            "form[action*='/redirect/sgo/'] > input[name='g-recaptcha-response'][value]",
            s
          )
        } else {
          s()
  ...
  domainBypass('brpaper.com', () =>
    safelyNavigate(location.href.replace('downloads', 'downloader'))
  )
  domainBypass('boo.tw', () =>
    ifElement('div#shorturl-go', d => {
  ...
  domainBypass(/(kora4top|forexlap)\.com/, () =>
    ifElement('div#m1x2 a', safelyNavigate)
  )
  ...
  hrefBypass(
    ...
    () => (location.href = 'serveRequest.php')
  )
  domainBypass('mboost.me', () =>
    ifElement('#__NEXT_DATA__', s =>
      safelyNavigate(
        JSON.parse(s.textContent).props.initialProps.pageProps.data.targeturl
      )
    )
  )
  domainBypass('go.geghost.com', () =>
    ifElement(
      "img[alt='Preview website'][src^='http://www.apercite.fr/api/apercite/320x200/oui/'",
      i => safelyNavigate(i.src.substr(48))
    )
  )
  domainBypass('shorte-st.online', () =>
    ifElement(
      '#cpt-form',
      f => {
        f.target = '_self'
  ...
  domainBypass('apunkasoftware.net', () =>
    ifElement('a#dlink[href]', safelyNavigate, () =>
      ifElement(
        "form#gip_form[action='https://www.apunkasoftware.net/download-process.php']",
        f => f.submit()
      )
    )
  )
  domainBypass('disingkat.in', () =>
    ifElement('a.redirect[href]', safelyNavigate, () => {
  ...
  domainBypass('shirosafe.web.id', () =>
    ifElement('#cus>a[href]', safelyNavigate)
  )
  domainBypass(/(techoow|histotechs)\.com/, () => {
    window.setTimeout = f => setTimeout(f, 1)
    window.setInterval = f => setInterval(f, 1)
    ifElement('a.btn-success[href]', safelyAssign, () =>
      ifElement(
        '#count00',
        a => {
  ...
  domainBypass('filesupload.org', () =>
    ifElement("a[href='?unlock']", safelyAssign, () =>
      ifElement('.download-timer', () =>
        awaitElement(".download-timer>form>input[name='link']", i =>
          safelyAssign(i.value)
        )
      )
    )
  )
  ...
  domainBypass('myotto.online', () =>
    ifElement('button#makingdifferenttimer > a[href]', safelyAssign)
  )
  domainBypass('disiniaja.site', () =>
    ifElement('button > a.button[href]', safelyAssign)
  )
  ...
  domainBypass('filehorse.com', () =>
    ifElement('a#download_url[href]', a => {
  ...
  domainBypass('catcut.net', () =>
    safelyNavigate(atob(new URL(go_url).searchParams.get('a')))
  )
  domainBypass('imgdrive.net', () =>
    ifElement('#redirect-wait', () => {
      $.post(
        location.href,
        {
          cti: 1,
  ...
  domainBypass('theartistunion.com', () =>
    awaitElement('.modal--download', () => {
      let xhr = new XMLHttpRequest()
      xhr.onload = () =>
        safelyNavigate(JSON.parse(xhr.responseText).audio_source)
      xhr.open('GET', '/api/v3' + location.pathname + '.json', true)
      countIt()
      xhr.send()
    })
  )
  domainBypass('boomx5.com', () =>
    ifElement('#form', f => {
  ...
  domainBypass(
    /customercareal\.com|(eduinstruct|medific|newswala)\.net/,
    () => {
      if (document.querySelector('.navbar-brand').innerText == 'Do2Unlock') {
        ...
        safelyNavigate(
          document
            .querySelectorAll('[type="text/javascript"]')[6]
            .innerText.split('f", "')[1]
            .split('")')[0]
        )
      }
  ...
  domainBypass('iloadit11.info', () =>
    ifElement('button#timerbtn', () => {
      let f = document.createElement('form')
      f.method = 'POST'
  ...
  domainBypass('sorewa.net', () =>
    ifElement(
      "p[style='text-align: center;'] > strong > a[href]",
      safelyNavigate
    )
  )
  hrefBypass(/akwam\.(net|in\/download)/, () =>
    ifElement(
      '.btn-loader > a.link.btn.btn-light[href][download]',
      safelyNavigate
    )
  )
  domainBypass(
    /^(lefturl|askquds|goo-2o)\.com|(palsweet|pluslive)\.live|go\.akwam\.in$/,
    () => ifElement('a.download-link[href]', safelyNavigate)
  )
  domainBypass('worldofmods.com', () =>
    ifElement('.repost-button-twitter', b => {
      window.open = _ => {}
      setTimeout(() => {
        b.click()
  ...
  domainBypass('linkconfig.com', () =>
    ifElement('a#download[href]', safelyNavigate)
  )
  domainBypass('suanoticia.online', () =>
    ifElement('#meio > a[href]', safelyNavigate)
  )
  hrefBypass(/crxne\.de\/crxneunlock\/crxne\/download\//, () =>
    safelyNavigate(
      'https://crxne.de/crxneunlock/crxne/download/links/downloadlinks.html'
    )
  )
  ...
  domainBypass('easylinkref.com', () =>
    ifElement('.easylinkref-go > strong', s => safelyNavigate(s.textContent))
  )
  domainBypass('subdowns.com', () =>
    ifElement('#botao', () => {
      window.setTimeout = f => setTimeout(f, 1)
  ...
  domainBypass('jk-chat.com', () =>
    safelyNavigate(atob(location.hash.substr(1)))
  )
  domainBypass('shorten.sh', () =>
    crowdBypass(() =>
      ifElement('#go-link', () =>
        awaitElement('#go-link.go-link', f =>
          $.post(f.action, $('#go-link').serialize(), d =>
            contributeAndNavigate(d.url)
          )
        )
      )
    )
  )
  domainBypass('urapk.com', () =>
    ifElement('#ed_dl_link > a[href]', safelyNavigate)
  )
  domainBypass('expertvn.com', () => {
    ...
    crowdBypass(() =>
      ifElement(
        ...
        () =>
          awaitElement('button#link:not([disabled])', b =>
            contributeAndNavigate(b.parentNode.href)
          )
      )
    )
  })
  ...
  domainBypass('mlwbd.pw', () => {
    document.querySelectorAll("tr[id^='mov']").forEach(t => {
      ...
      t.querySelector("a[href^='javascript']").href = t.querySelector(
        "input[name='FU']"
      ).value
    })
  })
  domainBypass('uploadking.net', () =>
    ifElement("form[name='F1']", f => countIt(() => f.submit()))
  )
  domainBypass('5play.ru', () =>
    ifElement('div.download-result > a[href]', safelyNavigate)
  )
  domainBypass('daunshorte.kertashitam.com', () =>
    ifElement('div[align=center] > center > a[href]', safelyAssign)
  )
  hrefBypass(/www1\.swatchseries\.to\/freecale\.html\?r\=/, () =>
    awaitElement(
      "a.push_button.blue[href]:not([href='http://www1.swatchseries.to/'])",
      safelyNavigate
    )
  )
  domainBypass('tl.gd', () =>
    safelyAssign('http://www.twitlonger.com/show' + location.pathname)
  )
  ...
  domainBypass('welcome.indihome.co.id', () =>
    ifElement(`.button-lanjut:not([onclick='stop_timer();'])`, a => a.click())
  )
  domainBypass('techrfour.com', () =>
    ifElement("input[name='newwpsafelink']", i => {
      const { parse } = JSON,
        { searchParams } = new URL(parse(atob(i.value)).linkr)
      if (searchParams.has('safelink_redirect')) {
        const { safelink, second_safelink_url } = parse(
          atob(searchParams.get('safelink_redirect'))
        )
  ...
  domainBypass('dl.ocanoke.com', () => {
    ...
    crowdBypass(() =>
      ifElement(
        ...
        f => f.submit(),
        () =>
          awaitElement('a.get-link[href]:not(.disabled)', a =>
            contributeAndNavigate(a.href)
          )
      )
    )
  ...
  domainBypass('apkhubs.com', () =>
    ifElement('a#downloadbtn', a => {
      countdown(0)
      safelyNavigate(a.href)
    })
  )
  domainBypass('favpng.com', () =>
    ifElement('div#countdown', () =>
      safelyNavigate(
        'https://files.favpng.com/api_download.php?k=' +
  ...
  domainBypass('sh.st', () =>
    ifElement(
      ...
      () => (location.hostname = 'ceesty.com')
    )
  )
  domainBypass('apkpsp.xyz', () =>
    ifElement('span#dwto', s => {
      window.setInterval = f => setInterval(f, 1)
      s.click()
      awaitElement('span#goto > a[href]', safelyNavigate)
    })
  )
  domainBypass('maukredit.online', () =>
    document.getElementById('wpsafe-link').children[0].click()
  )
  ...
  domainBypass('forex1pro.com', () =>
    safelyAssign('https://fx4vip.com' + location.pathname)
  )
  ...
  domainBypass('oracle.com', () =>
    document.querySelectorAll('[data-file]').forEach(e => {
      ...
      let link = e.getAttribute('data-file'),
        jre8 = RegExp(
          'download.oracle.com/otn/java/jdk/8u([0-9]*)-b([0-9]*)/([a-z0-9]{32})/(.*)$',
          'g'
        ).exec(link)
      if (jre8 && jre8[3]) {
        ...
        e.onclick = () =>
          safelyNavigate(
            'https://javadl.oracle.com/webapps/download/GetFile/1.8.0_' +
              jre8[1] +
              '-b' +
              jre8[2] +
              '/' +
  ...
  hrefBypass(
    /((psarips\.(com|net|org|eu|in|one|xyz|uk))|(psa\.one))\/exit\//,
    () =>
      ifElement("form[name='redirect']", f => {
        window.stop()
        safelyAssign(
          f.action + '#bypassClipboard=psarips:' + location.pathname.substr(6)
        )
      })
  )
  ...
  domainBypass('jwfinancas.club', () =>
    ifElement('a.btn-primary[href]', safelyAssign)
  )
  domainBypass('saver.id', () =>
    ifElement("input[name='ouyeah']", i => safelyAssign(i.value))
  )
  ...
  domainBypass('links.shortenbuddy.com', () =>
    safelyAssign(location.href.replace('links.', ''))
  )
  domainBypass('anon.to', () => ifElement('#redirect_button', safelyNavigate))
  domainBypass('dl-protect1.co', () =>
    ifElement(
      '#form_link',
      f => f.submit(),
      () => ifElement('.lienet > a[href]', safelyNavigate)
    )
  )
  domainBypass('linkdoni.soft98.ir', () =>
    ifElement('.actions a[href].button.primary', safelyNavigate)
  )
  ...
  domainBypass('tecknity.com', () => {
    if (typeof counter != 'undefined') counter = 0
  })
  domainBypass('url.rizaldi.web.id', () =>
    ifElement('a#download_link[href]', a => safelyNavigate(a.href))
  )
  domainBypass('1bit.space', () => {
    if (typeof hcaptcha == 'object' && typeof apiCounter == 'object') {
      ...
      apiCounter.redirectTo(
        document.querySelector('button.button-element-verification')
      )
    }
  })
  ...
  domainBypass('gcloud.live', () => {
    if (
      typeof buildDownload == 'function' &&
      location.pathname.substr(0, 3) == '/f/'
    ) {
  ...
  domainBypass('pnd.money', () =>
    keepLooking(() =>
      ifElement('form#pnd_redirect_form, form#link-view', f => f.submit())
    )
  )
  domainBypass('5mod-file.ru', () => ifElement('#form', f => f.submit()))
  domainBypass('kazanclilink.com', () =>
    ifElement('a#baglantigit', a => safelyNavigate(a.href))
  )
  domainBypass('moddingunited.xyz', () => {
    if (typeof reverseString == 'function')
      safelyNavigate(reverseString(window.location.search.substring(1)))
  })
  ...
  domainBypass('droidfilehost.com', () => {
    if (typeof wt !== 'undefined') wt = 1
  })
  domainBypass('kooi.xyz', () => ifElement('a#link_download', safelyNavigate))
  domainBypass('usdb.animux.de', () =>
    ifElement('form#timeform', f => f.submit())
  )
  domainBypass('www.thegamesdownload.net', () =>
    awaitElement('#gid', btn => safelyAssign(btn.value))
  )
    ...
    setTimeout(() => {
      document.querySelectorAll('script').forEach(script => {
        ...
        if (result && result.length !== 0) {
          eval(result[0])
          safelyAssign(
            redirectLink + (ignoreCrowdBypass ? '#ignoreCrowdBypass' : '')
          )
        }
      })
    }, 500)
  ...
  domainBypass('adbull.me', () =>
    ifElement('form#setc', ({ action }) => {
      unsafelyAssignWithReferer(location.href, action)
    })
  )
  domainBypass('tinyurl.is', () =>
    ifElement('a#skip-btn[href]', safelyNavigate)
  )
  ...
  domainBypass('duit.cc', () => {
    ...
    if (location.pathname == '/getlink.php')
      setTimeout(() => location.reload(), 1000)
  })
  domainBypass('animestc.xyz', () => {
    ifElement('#link-id', a => {
      fetch('https://protetor.animestc.xyz/api/link/' + a.getAttribute('value'))
        .then(r => r.json())
        .then(json => {
  ...
  domainBypass('egao.in', () => {
    ifElement('#SafelinkChecker', button => {
      fetch('https://egao.in/safelink', {
        ...
        .then(r => r.json())
        .then(j => safelyNavigate(j.data.url))
    })
  })
      ...
      safelyNavigate(
        decodeURIComponent(
          JSON.parse(
            atob(
              new URLSearchParams(
                JSON.parse(atob(input.value)).linkr.substr(25)
              ).get('safelink_redirect')
            )
          ).safelink
        )
      ) // lmaoooo
  ...
  domainBypass('apkdone.com', () => {
    ifElement('#download', a => {
      ensureDomLoaded(() => {
        countdown(0)
        safelyNavigate(
          document.querySelector('#download > a:nth-child(1)').href
        )
      })
    })
  })
  ...
  //Adf.ly "Locked" Page
  if (
    location.pathname == '/ad/locked' &&
    document.getElementById('countdown') &&
    document.querySelector('a').textContent == 'Click here to continue'
  ) {
    let wT = setInterval(() => {
  ...
  if (
    document.getElementById('adfly_bar') !== null &&
    typeof urlb == 'string'
  ) {
    safelyNavigate(urlb)
  }
  ...
  if (
    location.pathname.substr(0, 13) == '/redirecting/' &&
    document.querySelector('p[style]').textContent ==
  ...
  for (let domain in soralink_data) {
    domainBypass(domain, () =>
      document
        .querySelectorAll(
          "a[href^='" + location.origin + '?' + soralink_data[domain] + "=']"
        )
        .forEach(a => {
          a.href +=
            '#bypassClipboard=' +
            a.href.split('?' + soralink_data[domain] + '=')[1]
        })
    )
  }
  domainBypass('www.tech2learners.com', () =>
    safelyNavigate(downloadButton.href)
  )
  domainBypass('channelmyanmar.org', () =>
    document
      .querySelectorAll("a[href^='https://channelmyanmar.org?1c17f28bf0=']")
      .forEach(a => {
        if (a.classList.contains('FLMBTN-Btn')) {
          ...
          a.href +=
            '#bypassClipboard=' +
            location.pathname.replace(/[^a-zA-Z0-9]/g, '') +
            a.parentNode.firstChild.textContent
              .toLowerCase()
              .split(' ')
              .join('')
              .split('–')
  ...
  domainBypass('ad4msan.com', () =>
    document
      .querySelectorAll("a[href^='https://ad4msan.com?9c2a6bf968=']")
      .forEach(a => {
  ...
  domainBypass('oppa.kdramaindo.tv', () =>
    document
      .querySelectorAll("a[href^='https://oppa.kdramaindo.tv?38971fecb6=']")
      .forEach(a => {
        a.href +=
          '#bypassClipboard=' +
          location.pathname.replace(/[^a-zA-Z0-9]/g, '') +
  ...
  domainBypass('mkvking.com', () =>
    document
      .querySelectorAll("a[href^='https://mkvking.com?c17421bdaf=']")
      .forEach(a => {
        a.href +=
          '#bypassClipboard=' +
          a
  ...
  domainBypass('oploverz.in', () =>
    document.querySelectorAll("a[href*='?id=']").forEach(a => {
      ...
      if (ld) {
        ...
        a.href +=
          '#bypassClipboard=' +
          qe.textContent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() +
          a.textContent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()
      }
    })
  )
  domainBypass('neonime.moe', () =>
    document
      .querySelectorAll("a[href^='https://neonime.moe?700ef7c050=']")
      .forEach(a => {
        a.href +=
          '#bypassClipboard=' +
          location.pathname.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() +
          a.parentNode.firstChild.textContent
            .replace(/[^a-zA-Z0-9]/g, '')
            .toLowerCase() +
          a.textContent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()
  ...
  domainBypass('mmsubs.com', () =>
    document
      .querySelectorAll("a[href^='https://mmsubs.com?e75fad73d9=']")
      .forEach(a => {
        ...
        if (qe !== null) {
          a.href +=
            '#bypassClipboard=' +
            qe.textContent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() +
            a.parentNode.previousElementSibling.textContent.toLowerCase() +
            a.textContent.toLowerCase()
        }
      })
  )
  domainBypass('bakadame.com', () =>
    document
      .querySelectorAll("a[href^='https://bakadame.com?e41b7e5034=']")
      .forEach(a => {
        let qe = a.previousElementSibling
        ...
        if (qe !== null) {
          a.href +=
            ...
            a
              .closest('div')
              .previousElementSibling.textContent.replace(/[^a-zA-Z0-9]/g, '')
              .toLowerCase() +
            qe.textContent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() +
            a.textContent.toLowerCase()
        } else {
  ...
  domainBypass('drivenime.com', () =>
    document.querySelectorAll("a[href*='?a82ad005b1=']").forEach(a => {
      let p = a.parentNode,
        qe = p.previousElementSibling
      while (qe && qe.tagName != 'H2') {
        qe = qe.previousElementSibling
      }
      a.href +=
        '#bypassClipboard=' +
        p.textContent.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()
      if (p.parentNode.classList.contains('post-single-content')) {
        ...
        a.href += p.parentNode.previousElementSibling.textContent
          .replace(/[^a-zA-Z0-9]/g, '')
          .toLowerCase()
      }
    })
  )
  domainBypass('katmoviehd.nl', () =>
    document
      .querySelectorAll("a[href^='https://katmoviehd.nl?6de4d3b1de=']")
      .forEach(a => {
        a.href +=
          '#bypassClipboard=' +
          location.pathname.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() +
  ...
  domainBypass('hienzo.com', () =>
    document
      .querySelectorAll("a[href^='https://www.losstor.com/?id=']")
      .forEach(a => {
        a.href +=
          '#bypassClipboard=' +
          location.pathname.replace(/[^a-zA-Z0-9]/g, '').toLowerCase() +
  ...
  domainBypass('animebatch.id', () =>
    document
      .querySelectorAll("a[href^='https://tamatekno.net/?id=']")
      .forEach(a => {
        let ver = a.closest('ul').previousElementSibling,
          qe = ver,
          res = a.previousElementSibling
        ...
        a.href +=
          '#bypassClipboard=' +
          qe.textContent.replace(/[^\w]/g, '').toLowerCase() +
          res.textContent.replace(/[^\w]/g, '').toLowerCase() +
          ver.textContent.replace(/[^\w]/g, '').toLowerCase() +
          a.textContent.replace(/[^\w]/g, '').toLowerCase()
      })
  )
  domainBypass('koenime.com', () =>
    document
      .querySelectorAll("a[href^='https://apasih.pw/?id=']")
      .forEach(a => {
        let qe = a.closest('ul').previousElementSibling,
          ep = a.closest('.dlmn321').previousElementSibling
        a.href +=
          '#bypassClipboard=' +
          location.pathname.replace(/[^\w]/g, '') +
          qe.textContent.replace(/[^\w]/g, '').toLowerCase() +
          ep.textContent.replace(/[^\w]/g, '').toLowerCase() +
          a.textContent.replace(/[^\w]/g, '').toLowerCase()
      })
  )
  domainBypass('masteredutech.com', () =>
    safelyAssign(document.querySelector('.btn-captcha').href)
  )
  domainBypass('bakacan.id', () =>
    document
      .querySelectorAll("a[href^='https://apasih.pw/?id=']")
      .forEach(a => {
        let qe = a.previousElementSibling
        ...
        if (qe !== null) {
          a.href +=
            a
              .closest('ul')
              .previousElementSibling.textContent.replace(/[^\w]/g, '')
              .toLowerCase() +
            qe.textContent.replace(/[^\w]/g, '').toLowerCase()
        } else {
          a.href += a.parentNode.previousElementSibling.textContent
            .replace(/[^\w]/g, '')
            .toLowerCase()
        }
  ...
  if (document.querySelector('#goes > #lanjut > a[href]')) {
    //#165
  ...
  if (
    typeof bukalink == 'function' &&
    document.getElementById('bijil1') &&
    document.getElementById('bijil2')
  ) {
    //gosavelink.com
    window.open = safelyNavigate
    bukalink()
  ...
  if (typeof changeLink == 'function') {
    ifElement(
      ...
      () => {
        let cLT = setInterval(() => {
          if (
            (document.querySelector('img#pleasewait') &&
  ...
  if (document.querySelector('form#landing')) {
    ...
    if (bypassClipboard) {
      ...
      document.querySelector('form#landing > div#landing') &&
      document.querySelector('.soractrl')
    ) {
  ...
  if (
    bypassClipboard &&
    document.querySelector('img.spoint#showlink') &&
    document.querySelector('.soractrl')
  ) {
    if (!ignoreCrowdBypass) {
      insertInfoBox(
        crowdEnabled ? '{{msg.crowdWait}}' : '{{msg.crowdDisabled}}'
      )
    }
    const _ce = document.createElement
  ...
  ifElement(
    ".wpsafe-top > form > input.btn.btn-primary[type='submit'][value]",
    i => {
      i.click()
      finish()
    }
  ...
  if (
    document.getElementById('wpsafe-generate') &&
    typeof wpsafegenerate == 'function'
  ) {
    ifElement(
      '#wpsafegenerate > #wpsafe-link > a[href]',
      a => {
        safelyNavigate(a.href)
        return finish()
      },
      () => {
        ...
        if (s.has('go')) {
          if (safelyNavigate(atob(s.get('go')))) return finish()
  ...
  ifElement("form[action*='?safelink_redirect=']", f => {
    //#557
  ...
  if (
    document.querySelector("input[type='hidden'][name='newwpsafelink'][value]")
  ) {
    let s = new URLSearchParams(location.search)
    if (s.has('go')) {
  ...
  //wpapk template
  ifElement('a[href].wpapks-download-link', a =>
    ifElement('#wpapks-pre-download-btn', b => {
      b.onclick = () => safelyNavigate(a.href)
    })
  )
  //Other Templates
  ifElement('.timed-content-client_show_0_30_0', d => {
    //technicoz.com
  ...
  if (
    ...
    document.getElementById('gotolink') &&
  ...
  if (
    document.querySelector('#tungguyabro') &&
    typeof WaktunyaBro == 'number'
  ) {
    //short.mangasave.me
    WaktunyaBro = 0
    setInterval(() => {
  ...
  if (
    document.querySelector('#yangDihilangkan > a') &&
    document.querySelector('#downloadArea > .text-center')
  ) {
    //rathestation.bid
  ...
  if (
    document.querySelector('a.redirectBTN.disabled') &&
  ...
  if (
    document.querySelector(".shortened_link a[href][ng-href][target='_blank']")
  ) {
    //Go2to.com,Go2too.com,Golink.to
    safelyNavigate(
      document.querySelector(
        ".shortened_link a[href][ng-href][target='_blank']"
      ).href
    )
  }
  if (location.search.startsWith('?n=')) {
    //viralking.xyz,indian4uh.com
  ...
  if (
    typeof adblock_message == 'string' &&
    adblock_message.indexOf('mm1.ink') > -1 &&
  ...
  if (
    document.querySelector("#templatemo_footer > a[href*='teknosafe']") &&
    document.querySelector('#templatemo_content > div > a[href]')
  ) {
    //teknosafe.kertashitam.com,teknosafe.teknologilink.com
    safelyNavigate(
      document.querySelector('#templatemo_content > div > a[href]').href
    )
    return finish()
  }
  ifElement(
    "button#makingdifferenttimer[onclick^='window.location.replace']",
    b => {
      //#522
      b.click()
      finish()
  ...
  ifElement("input[type='hidden'][name='mylink'][value^='http']", i => {
    //#549
  ...
  if (typeof megabux == 'object' && 'link' in megabux) {
    //acortaz.com#1460
    ...
    if (location.pathname.substring(0, 6) == '/skip/') {
      crowdBypass(() => {
        ifElement("div.top-bar form[method='POST'][action]", f => {
          awaitElement(
            "div.top-bar form[method='POST'][action] #skip-button[value='Skip AD']",
            b => {
              f.action += '#bypassClipboard=' + location.pathname.substr(1)
              b.click()
            }
  ...
  ifElement("meta[property='og:site_name'][content='Adshrink.it']", () => {
    let iT = setInterval(() => {
      if (
        ...
        typeof ___reactjsD != 'undefined' &&
        typeof window[___reactjsD.o] == 'object' &&
        typeof window[___reactjsD.o].dest == 'string'
      ) {
        clearInterval(iT)
  ...
  domainBypass('techynroll.com', () =>
    awaitElement('a#enablebtn', a => safelyAssign(a.href))
  )
  hrefBypass(/meostream\.com\/links\//, () =>
    ifElement('a#link', safelyNavigate)
  )
  //XImageSharing
  ...
  if (!bypassed && t) {
    ...
    switch (t) {
      ...
      case 'AdFoc.us':
        ifElement('a.skip[href]', e => {
          safelyNavigate(e.href)
        })
        break
      ...
      case 'Glinks!':
        ifElement(
          "form[method='POST'] > input[type='submit'][name='btn'].btn",
          i => {
  ...
  let dT = setInterval(() => {
    ...
    if (typeof app_vars == 'object') {
      ...
      domainBypass('adcoinfly.com', () =>
        document
          .querySelectorAll("a.btn[href=''], a.btn[href*='clickme']")
      ...
      domainBypass(
        /movienear\.me|lewat\.club|tautan\.pro|(droidtamvan|gubukbisnis|onlinecorp)\.me|(liveshootv|modebaca|haipedia|sekilastekno|miuiku)\.com|shrink\.world|link\.mymastah\.xyz|(sportif|cararoot)\.id|healthinsider\.online/,
        () => {
          ifElement(
            "input[type='hidden'][name='alias'][value]",
            i => {
              i.parentNode.action +=
                '#' + i.value + (ignoreCrowdBypass ? '#ignoreCrowdBypass' : '')
              crowdPath(i.value)
            },
            () =>
              ifElement(
                ...
                f => {
                  f.action +=
                    ...
                    (ignoreCrowdBypass ? '#ignoreCrowdBypass' : '')
                  crowdPath(location.hash.substr(1))
                },
                () => crowdPath(location.hash.substr(1))
              )
      ...
      domainBypass(
        /(atv|adlink)\.pw|safe\.mirrordown\.com|kabarviral\.blog/,
        () => crowdPath(location.search.substr(1).split('=')[0])
      )
      docSetAttribute('{{channel.adlinkfly_info}}', '')
      let iT = setInterval(() => {
        if (
          document.documentElement.hasAttribute('{{channel.adlinkfly_target}}')
        ) {
          clearInterval(iT)
          let t = document.documentElement.getAttribute(
            '{{channel.adlinkfly_target}}'
          )
          if (t == '') {
            if (
              UNIVERSAL_BYPASS_INTERNAL_VERSION >= 10 &&
              bypassClipboard.substr(0, 8) == 'psarips:'
            ) {
            ...
            crowdBypass(() => {
              let cT = setInterval(() => {
                //                                                                                                                                                                               v spotted on fc-lc.com
                let a = document.querySelector(
                    "a.get-link[href]:not([href='']):not([href*='.ads.']):not([href*='//ads.']):not(.disabled), .skip-ad a[href]:not([href='']):not([href*='.ads.']):not([href*='//ads.']), a#surl[href]:not([href='']):not([href*='.ads.']):not([href*='//ads.']), a.pnd-submit-button[href]:not([href^='javascript:'])"
                  ),
                  h
                if (a) {
                  h = a.href
                ...
                if (isGoodLink(h)) {
                  if (h.indexOf('://partners.popcent.net/') == -1) {
                    //tr.link
                    clearInterval(cT)
                    a.parentNode.removeChild(a)
      ...
      domainBypass(
        /123l\.pw|123link|(linksht|icutlink)\.com/,
        () => (window.setInterval = f => setInterval(f, 1))
      )
      clearInterval(dT)
    }
    ...
    if (typeof redirectpage != 'undefined' && typeof CryptoJS != 'undefined') {
      document
        .querySelectorAll("a[href^='" + redirectpage + "']")
        .forEach(a => {
          let url = CryptoJS.AES.decrypt(
            atob(new URL(a.href).searchParams.get('token')),
            '391si8WU89ghkDB5'
          ).toString(CryptoJS.enc.Utf8)
          if (isGoodLink(url)) {
            a.href = url
