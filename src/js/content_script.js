...
if (document instanceof HTMLDocument) {
  let clipboardIndex = location.hash.indexOf('#bypassClipboard='),
  ...
  if (clipboardIndex != -1) {
    ...
    location.hash = location.hash.substr(
      0,
      location.hash.length - bypassClipboard.length - 17
    )
  }
  if (location.hash.substr(-18) == '#ignoreCrowdBypass') {
    ignoreCrowdBypass = true
  ...
  brws.runtime.sendMessage({ type: 'content' }, res => {
    ...
    const channel = res.channel,
      observer = new MutationObserver(mutations => {
        if (document.documentElement.hasAttribute(channel.stop_watching)) {
          ...
          document.documentElement.hasAttribute(channel.crowd_referer)
        ) {
          ...
          document.documentElement.hasAttribute(channel.crowd_domain)
        ) {
          domain = simplifyDomain(
            document.documentElement.getAttribute(channel.crowd_domain)
          )
          ...
          port.onMessage.addListener(msg => {
            if (msg == '') {
              ...
              location.assign(
                'https://universal-bypass.org/crowd-bypassed?target=' +
          ...
          document.documentElement.hasAttribute(channel.crowd_contribute)
        ) {
          const target = document.documentElement.getAttribute(
            channel.crowd_contribute
          )
          ...
          brws.runtime.sendMessage({
            ...
            data:
              ...
              '&path=' +
              encodeURIComponent(crowdPath) +
              '&target=' +
              encodeURIComponent(target)
          })
        } else if (
          document.documentElement.hasAttribute(channel.adlinkfly_info)
        ) {
          ...
          document.documentElement.hasAttribute(channel.bypass_clipboard)
        ) {
          const clipboard = document.documentElement.getAttribute(
            channel.bypass_clipboard
          )
    ...
    script.innerHTML =
      `(()=>{
			const crowdEnabled=` +
      (res.crowdEnabled ? 'true' : 'false') +
      `,
			ignoreCrowdBypass=` +
      (ignoreCrowdBypass ? 'true' : 'false') +
      `,
			bypassClipboard="` +
      bypassClipboard
        .split('\\')
