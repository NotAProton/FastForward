const brws = typeof browser == 'undefined' ? chrome : browser,
  ...
  getRedirect = (url, referer, safe_in) => {
    ...
    let redirectUrl =
      brws.runtime.getURL('html/before-navigate.html') +
      '?target=' +
  ...
  encodedRedirect = (url, referer, safe_in) =>
    getRedirect(decodeURIComponent(url), referer, safe_in),
  isGoodLink = link => {
    if (
      !link ||
...
var bypassCounter = 0,
  ...
  instantNavigationTrackers = false,
  blockIPLoggers = true,
  crowdEnabled = true,
  userScript = ''
...
brws.storage.onChanged.addListener(changes => {
  ...
  if (changes.no_instant_navigation_trackers) {
    instantNavigationTrackers =
      changes.no_instant_navigation_trackers.newValue !== 'true'
  }
...
const updateBypassDefinitions = callback => {
    ...
    const finishDownload = () => {
      ...
      ;[
        ...
        uniqueness.push(val)
        upstreamInjectionScript = upstreamInjectionScript
          .split('{{channel.' + name + '}}')
          .join((channel[name] = 'data-' + val))
      })
      ;[
        'infoLinkvertise',
        'infoFileHoster',
      ...
      upstreamInjectionScript = upstreamInjectionScript
        .split('{{icon/48.png}}')
        .join(brws.runtime.getURL('icon/48.png'))
      refreshInjectionScript()
      updateStatus = ''
      ...
      xhr.onerror = () => {
        ...
        xhr.onload = () => {
          ...
          if (latestCommit == upstreamCommit) {
            ...
            xhr.open(
              'GET',
              'https://raw.githubusercontent.com/FastForwardTeam/FastForward/' +
                upstreamCommit +
        ...
        xhr.open(
          'GET',
          'https://api.github.com/repos/FastForwardTeam/FastForward/commits/main',
          true
        )
        xhr.send()
      }
  ...
  refreshInjectionScript = () => {
    Object.values(onBeforeRequest_rules).forEach(func =>
      brws.webRequest.onBeforeRequest.removeListener(func)
    )
    Object.values(onBeforeSendHeaders_rules).forEach(func =>
      brws.webRequest.onBeforeSendHeaders.removeListener(func)
    )
    Object.values(onHeadersReceived_rules).forEach(func =>
      brws.webRequest.onHeadersReceived.removeListener(func)
    )
    if (enabled) {
      injectionScript = (upstreamInjectionScript + '\n' + userScript)
        .split('UNIVERSAL_BYPASS_INTERNAL_VERSION')
        .join('10')
        .split('UNIVERSAL_BYPASS_EXTERNAL_VERSION')
        .join(extension_version)
        .split('UNIVERSAL_BYPASS_INJECTION_VERSION')
        .join(upstreamCommit ? upstreamCommit.substr(0, 7) : 'dev')
      Object.keys(preflightRules).forEach(name => {
        if (name in onBeforeRequest_rules) {
          brws.webRequest.onBeforeRequest.addListener(
            onBeforeRequest_rules[name],
            { types: ['main_frame'], urls: preflightRules[name] },
            ['blocking']
          )
        } else if (name in onBeforeSendHeaders_rules) {
          brws.webRequest.onBeforeSendHeaders.addListener(
            onBeforeSendHeaders_rules[name],
            {
              types: ['main_frame', 'xmlhttprequest'],
              urls: preflightRules[name]
            },
            ['blocking', 'requestHeaders']
          )
        } else if (name in onHeadersReceived_rules) {
          brws.webRequest.onHeadersReceived.addListener(
            onHeadersReceived_rules[name],
            { types: ['main_frame'], urls: preflightRules[name] },
  ...
  switch (req.type) {
    ...
    case 'crowd-contribute':
      if (crowdEnabled) {
        ...
        xhr.open(
          'POST',
          'https://universal-bypass.org/crowd/contribute_v1',
          true
        )
  ...
  switch (port.name) {
    case 'options':
      ...
      port.onMessage.addListener(req => {
        switch (req.type) {
          case 'update':
            updateBypassDefinitions(updateSuccess =>
              port.postMessage({ updateSuccess })
            )
            break
        }
      })
    ...
    case 'crowd-query':
      port.onMessage.addListener(msg => {
        ...
        xhr.send(
          'domain=' +
            encodeURIComponent(msg.domain) +
            '&path=' +
            encodeURIComponent(msg.crowdPath)
        )
      })
      break
      ...
      port.onMessage.addListener(msg => {
        ...
        xhr.onload = () => {
          let i = new DOMParser()
            ...
            .querySelector("img[src^='//api.miniature.io']")
          if (i) {
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  { types: ['main_frame'], urls: ['*://universal-bypass.org/firstrun?0'] },
  ['blocking']
)
brws.webRequest.onBeforeRequest.addListener(
  details => {
  ...
  { types: ['main_frame'], urls: ['*://universal-bypass.org/firstrun?1'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  {
    types: ['main_frame'],
    urls: ['*://universal-bypass.org/bypassed?target=*&referer=*']
  },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  details => {
    ...
    return {
      redirectUrl:
        brws.runtime.getURL('html/crowd-bypassed.html') + details.url.substr(43)
    }
  },
  {
    types: ['main_frame'],
    urls: ['https://universal-bypass.org/crowd-bypassed?*']
  },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  details => {
    return {
      redirectUrl:
        brws.runtime.getURL('html/options.html') + details.url.substr(36)
    }
  },
  { types: ['main_frame'], urls: ['https://universal-bypass.org/options*'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  {
    types: ['main_frame'],
    urls: ['*://universal-bypass.org/navigate?target=*']
  },
  ['blocking']
)
...
brws.webRequest.onBeforeRedirect.addListener(
  details => {
    if (enabled && details.url in refererCache) {
...
brws.webRequest.onCompleted.addListener(
  details => {
    if (enabled && details.url in refererCache) {
      delete refererCache[details.url]
    }
...
function resolveRedirect (url) {
  ...
  xhr.onload = () => {
    ...
    if (json && json.destination) {
      ...
      preflightRules.tracker_force_http.forEach(domain => {
        if (
          destination.substr(0, 9 + domain.length) ==
          'https://' + domain + '/'
        ) {
          destination =
            'http://' + domain + '/' + destination.substr(9 + domain.length)
        }
      })
    }
  }
  xhr.open(
    'GET',
    'https://apimon.de/redirect/' + encodeURIComponent(url),
    false
  )
  xhr.send()
  return destination
...
const onBeforeRequest_rules = {
    path_base64: details =>
      getRedirect(atob(details.url.substr(details.url.indexOf('aHR0c')))),
    path_s_encoded: details =>
      encodedRedirect(details.url.substr(details.url.indexOf('/s/') + 3)),
    path_r_base64: details =>
      getRedirect(atob(details.url.substr(details.url.indexOf('/r/') + 3))),
    path_dl_base64: details =>
      getRedirect(atob(details.url.substr(details.url.indexOf('/dl/') + 4))),
    path_ads_hex: details =>
      getRedirect(
        decodeHexString(details.url.substr(details.url.indexOf('/ads/') + 5))
      ),
    ...
    query_base64: details =>
      getRedirect(atob(new URL(details.url).search.replace('?', ''))),
    hash_base64: details => {
      let url = new URL(details.url)
      if (url.hash) {
    ...
    param_url_general: details => {
      ...
      if (url.substr(0, 5) == 'aHR0c' || url.substr(0, 7) == 'bWFnbmV') {
        ...
        url.substr(0, 13) == 'http%3A%2F%2F' ||
        url.substr(0, 14) == 'https%3A%2F%2F'
      ) {
    ...
    param_url_raw: details =>
      getRedirect(details.url.substr(details.url.indexOf('?url=') + 5)),
    ...
    param_xurl_raw_http: details =>
      getRedirect(
        'http' + details.url.substr(details.url.indexOf('?xurl=') + 6)
      ),
    ...
    param_rel_base64: details =>
      getRedirect(atob(details.url.substr(details.url.indexOf('?rel=') + 5))),
    param_link_encoded: details =>
      encodedRedirect(details.url.substr(details.url.indexOf('link=') + 5)),
    ...
    param_link_encoded_base64: details =>
      getRedirect(
        decodeURIComponent(
          atob(details.url.substr(details.url.indexOf('?link=') + 6))
        )
      ),
    param_kesehatan_base64: details =>
      getRedirect(
        atob(details.url.substr(details.url.indexOf('?kesehatan=') + 11))
      ),
    param_wildcard_base64: details =>
      getRedirect(
        atob(new URL(details.url).searchParams.values().next().value)
      ),
    param_r_base64: details =>
      getRedirect(
        atob(
          decodeURIComponent(details.url.substr(details.url.indexOf('?r=') + 3))
        ),
        0,
        safe_in
      ),
    param_kareeI_base64_pipes: details =>
      getRedirect(
        atob(details.url.substr(details.url.indexOf('?kareeI=') + 8)).split(
          '||'
        )[0]
      ),
    ...
    param_a_base64: details =>
      getRedirect(
        atob(
          details.url.substr(details.url.indexOf('?a=') + 3).split('#', 1)[0]
        )
      ),
    ...
    param_id_base64: details =>
      getRedirect(atob(details.url.substr(details.url.indexOf('?id=') + 4))),
    ...
    param_go_base64: details => {
      let b64 = details.url
        .substr(details.url.indexOf('?go=') + 4)
        .split('&')[0]
    ...
    param_site_base64: details =>
      getRedirect(
        atob(
          details.url.substr(details.url.indexOf('?site=') + 6).split('&')[0]
        )
      ),
    param_reff_base64: details =>
      getRedirect(atob(details.url.substr(details.url.indexOf('?reff=') + 6))),
    param_s_encoded: details =>
      encodedRedirect(
        details.url.substr(details.url.indexOf('?s=') + 3),
        details.url
      ),
    param_dl_encoded_base64: details =>
      encodedRedirect(
        atob(details.url.substr(details.url.indexOf('?dl=') + 4).split('&')[0])
      ),
    param_health_encoded_base64: details =>
      encodedRedirect(
        atob(details.url.substr(details.url.indexOf('?health=') + 8))
      ),
            ...
            if (
              ...
              xhr.send(
                'domain=' +
                  url.host +
                  '&path=' +
                  encodeURIComponent(url.hash.substr(1)) +
                  '&target=' +
  ...
  details => {
    if (enabled) {
      return encodedRedirect(
        details.url.substr(details.url.indexOf('/12/1/') + 6)
      )
    }
  },
  { types: ['main_frame'], urls: ['*://*.sh.st/r/*/12/1/*'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  details => {
    if (enabled) {
      return getRedirect(
        details.url.substr(details.url.substr(16).indexOf('/') + 17)
      )
    }
  },
  { types: ['main_frame'], urls: ['http://sh.st/st/*/*'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  { types: ['main_frame'], urls: ['http://gslink.co/a/*'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  { types: ['main_frame'], urls: ['*://*.surfsees.com/?*'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  details => {
    if (enabled) {
      const arg_index = details.url.indexOf('&soralink_contribute='),
        url = details.url.substr(0, arg_index)
      if (crowdEnabled) {
  ...
  { types: ['main_frame'], urls: ['*://*/?*=*&soralink_contribute=*'] },
  ['blocking']
)
  ...
  details => {
    if (enabled) {
        ...
        if (details.method == 'POST') {
          for (let i in details.responseHeaders) {
            ...
            if (
              ...
              xhr.send(
                'domain=' +
                  new URL(details.url).host +
                  '&path=' +
                  encodeURIComponent(soralink_contribute[details.url]) +
                  '&target=' +
                  encodeURIComponent(header.value)
              )
              break
      ...
      if (firefox) {
        ...
        for (let i in details.responseHeaders) {
          if (
            'value' in details.responseHeaders[i] &&
            ['content-security-policy', 'x-content-security-policy'].indexOf(
              details.responseHeaders[i].name.toLowerCase()
            ) > -1
          ) {
            csp = true
            let _policies = details.responseHeaders[i].value
                .replace(';,', ';')
                .split(';'),
              policies = {}
            for (let j in _policies) {
              let policy = _policies[j].trim(),
                name = policy.split(' ')[0]
